...
public class CreateBucket {
    ...
    public static void CriarBucket(String nome) throws IOException {
        ...
        try {
            ...
            if (!s3Client.doesBucketExistV2(nome)) {
                // Because the CreateBucketRequest object doesn't specify a region, the
                // bucket is created in the region specified in the client.
                //s3Client.createBucket(new CreateBucketRequest(bucketName));
                
                CreateBucketRequest request = new CreateBucketRequest(nome);
                Bucket bucket = s3Client.createBucket(request);

                // Verify that the bucket was created by retrieving it and checking its location.
                String bucketLocation = s3Client.getBucketLocation(new GetBucketLocationRequest(nome));
